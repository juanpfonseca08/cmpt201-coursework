...
5 fixed string messages to the server.
Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
run_acceptor thread receives a pointer to struct acceptor_args.
2. How are received messages stored?
in a shared linked list.
3. What does `main()` do with the received messages?
intitializes the mutex and shared list, starts acceptor thread, waits untill all
expected messages are received, stops thread, and calls collect_all() to print
and free msgs.
4. How are threads used in this sample code?
main thread controls whole program, one acceptor thread accepts incoming client
connections, ont client thread created for each connected client to reach
messages concurrently and add them to the shared list.
*/
